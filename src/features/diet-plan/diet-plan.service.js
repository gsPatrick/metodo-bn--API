    ...
    const created = await DietPlan.create(
      {
        patientProfileId,
        nutritionistId: profile.nutritionistId,
        title: title || `Plano alimentar${parsed.prescritoEm ? ` · ${parsed.prescritoEm}` : ''}`,
        ...
        shoppingItems: parsed.shopping && parsed.shopping.length ? parsed.shopping : null,
        // Já sobe aprovado: o paciente vê o plano assim que a nutri envia.
        status: DIET_PLAN_STATUS.APPROVED,
        approvedAt: new Date(),
      },
      { transaction: t },
    );
